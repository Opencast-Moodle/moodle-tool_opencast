...
use tool_opencast\local\addactivity_form;
...
$redirecturloverview = new moodle_url('/admin/tool/opencast/index.php', ['courseid' => $courseid, 'ocinstanceid' => $ocinstanceid]);
...
if (activitymodulemanager::is_enabled_and_working_for_series($ocinstanceid) == false) {
    throw new moodle_exception('add opencast activity series module not enabled or working', 'tool_opencast', $redirecturloverview);
}
...
if ($moduleid) {
    ...
    redirect($redirecturloverview, get_string('addactivity_moduleexists', 'tool_opencast'), null, notification::NOTIFY_WARNING);
}

$addactivityform = new addactivity_form(null, ['courseid' => $courseid, 'ocinstanceid' => $ocinstanceid, 'seriesid' => $seriesid]);
...
if ($data = $addactivityform->get_data()) {
    ...
    if (
        get_config('tool_opencast', 'addactivityavailability_' . $ocinstanceid) != true ||
        empty($CFG->enableavailability) ||
        !isset($data->availabilityconditionsjson) ||
    ...
    if ($ocseries == null) {
        $str = get_string('series_not_found', 'tool_opencast', $data->seriesid);
        redirect($redirecturloverview, $str, null, notification::NOTIFY_ERROR);
    }

    // Create the module.
    $result = activitymodulemanager::create_module_for_series(
        $courseid,
        $ocinstanceid,
        $data->title,
        $seriesid,
    ...
    if ($result == true) {
        $str = get_string('addactivity_modulecreated', 'tool_opencast', $data->title);
        // Form was submitted with second submit button.
        if ($submitbutton2) {
            // Redirect to course overview.
            redirect($redirecturlcourse, $str, null, notification::NOTIFY_SUCCESS);
            // Form was submitted with first submit button.
        } else {
            // Redirect to Opencast videos overview page.
            redirect($redirecturloverview, $str, null, notification::NOTIFY_SUCCESS);
        }
        // Otherwise.
    } else {
        // Redirect to Opencast videos overview page.
        $str = get_string('addactivity_modulenotcreated', 'tool_opencast', $data->title);
        redirect($redirecturloverview, $str, null, notification::NOTIFY_ERROR);
    }
}
