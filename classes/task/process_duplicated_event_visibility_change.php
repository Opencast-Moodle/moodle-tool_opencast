    ...
    public function execute() {
        ...
        try {
            ...
            if (empty($data->ocworkflowid) || !is_number($data->ocworkflowid)) {
                ...
                throw new moodle_exception('error_no_duplicate_workflow_id', 'tool_opencast', '', $a);
            }
            // Event checks.
            if (empty($data->sourceeventid)) {
                $visiblitychangestatus = self::TASK_FAILED;
                throw new moodle_exception('error_no_duplicate_origin_event_id', 'tool_opencast', '', $a);
            }
            ...
            if ($retry) {
                ...
                throw new moodle_exception('errorduplicatedeventvisibilitytaskretry', 'tool_opencast', '', $e->getMessage());
            } else {
