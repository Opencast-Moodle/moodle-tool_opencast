...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=void 0,Ajax=_interopRequireWildcard(Ajax),Notification=_interopRequireWildcard(Notification),Str=_interopRequireWildcard(Str),Toast=_interopRequireWildcard(Toast);_exports.init=()=>{Str.get_strings([{key:"maintenancemode_modal_sync_confirmation_title",component:"tool_opencast"},{key:"maintenancemode_modal_sync_confirmation_text",component:"tool_opencast"},{key:"maintenancemode_modal_sync_confirmation_btn",component:"tool_opencast"},{key:"maintenancemode_modal_sync_error_title",component:"tool_opencast"},{key:"maintenancemode_modal_sync_error_noinstance_message",component:"tool_opencast"},{key:"maintenancemode_modal_sync_failed",component:"tool_opencast"},{key:"maintenancemode_modal_sync_succeeded",component:"tool_opencast"}]).then((function(jsstrings){document.querySelectorAll(".form-setting .opencast_config_dt_selector").forEach((dtblock=>{var _dtblock$dataset;if(null!=dtblock&&null!==(_dtblock$dataset=dtblock.dataset)&&void 0!==_dtblock$dataset&&_dtblock$dataset.isoptional){const enablingelement=document.getElementById(`${dtblock.dataset.settingid}_enabled`),initialvalue=(null==enablingelement?void 0:enablingelement.checked)??!1,selects=dtblock.querySelectorAll(".opencast-config-dt-select");selects.forEach((select=>{select.disabled=!initialvalue})),enablingelement.addEventListener("change",(event=>{selects.forEach((select=>{select.disabled=!event.target.checked}))}))}}));document.querySelectorAll(".maintenance-sync-btn").forEach((btn=>{btn.addEventListener("click",(e=>{var _e$target,_e$target$dataset;e.preventDefault();const ocinstanceid=null===(_e$target=e.target)||void 0===_e$target||null===(_e$target$dataset=_e$target.dataset)||void 0===_e$target$dataset?void 0:_e$target$dataset.ocinstanceid;ocinstanceid?Notification.confirm(jsstrings[0],jsstrings[1],jsstrings[2],null,(()=>{Ajax.call([{methodname:"tool_opencast_maintenance_sync",args:{ocinstanceid:ocinstanceid}}])[0].then((data=>{null!=data&&data.status?(Toast.add(jsstrings[6],{type:"success"}),setTimeout((function(){window.location.reload()}),3e3)):Toast.add(jsstrings[5],{type:"danger"})})).catch((error=>Notification.exception(error)))})):Notification.alert(jsstrings[3],jsstrings[4])})),btn.removeAttribute("disabled"),btn.removeAttribute("title"),btn.classList.remove("disabled"),btn.classList.remove("btn-warning"),btn.classList.add("btn-primary")}))})).catch(Notification.exception)}}));

//# sourceMappingURL=tool_maintenance.min.js.map