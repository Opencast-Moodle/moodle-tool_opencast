...
use tool_opencast\local\importvideos_coursesearch;
use tool_opencast\local\importvideos_select_series_form;
use tool_opencast\local\importvideos_step1_form;
use tool_opencast\local\importvideos_step2_form;
use tool_opencast\local\importvideos_step3_form;
use tool_opencast\local\importvideos_step3_form_acl;
use tool_opencast\local\importvideos_step4_form;
...
// Set base URL.
$baseurl = new moodle_url('/admin/tool/opencast/importvideos.php', ['courseid' => $courseid, 'ocinstanceid' => $ocinstanceid]);
...
if (
    (importvideosmanager::handle_series_modules_is_enabled_and_working($ocinstanceid) == true &&
...
switch ($step) {
    ...
    case 1:
        ...
        if ($possiblesourcecoursescount < 1 || ($possiblesourcecoursescount == 1 && $possiblesourcecourses[0]->id == $courseid)) {
            // Use step 1 form.
            $importvideosform = new importvideos_step1_form(
                null,
                ['courseid' => $courseid]
            );
        ...
        // Intentional fallthrough to next case if nextstep is true.
    ...
    case 2:
        ...
        if ($importmode == 'duplication') {
            ...
            $importvideosform = new importvideos_step2_form(
                null,
                [
            ...
            $courseseries = importvideosmanager::get_import_source_course_series(
                $ocinstanceid,
                $sourcecourseid
            );
    ...
    case 3:
        ...
        if (!$jumpto4) {
            if ($importmode == 'acl') {
                $importvideosform = new importvideos_step3_form_acl(
                    null,
                    [
                        'ocinstanceid' => $ocinstanceid,
                        'courseid' => $courseid,
                ...
                $importvideosform = new importvideos_step3_form(
                    null,
                    [
                        'ocinstanceid' => $ocinstanceid,
                        'courseid' => $courseid,
            ...
            if ($data = $importvideosform->get_data()) {
                if ($importmode == 'acl') {
                    // Perform ACL change.
                    $resultaclchange = importvideosmanager::change_acl(
                        $ocinstanceid,
                        $sourcecourseid,
                        $data->sourcecourseseries,
                        $courseid
                    );
    ...
    case 4:
        $step = 4;
        // Use step 4 form.
        $importvideosform = new importvideos_step4_form(
            null,
            [
                'ocinstanceid' => $ocinstanceid,
                'courseid' => $courseid,
        ...
        // Process data.
        if ($data = $importvideosform->get_data()) {
            $resultduplicate = importvideosmanager::duplicate_videos(
                $ocinstanceid,
                $sourcecourseid,
                $courseid,
                $coursevideos,
            ...
            if ($fixseriesmodules == true) {
                ...
                if (
                    ...
                    $resulthandleseries = ltimodulemanager::cleanup_series_modules(
                        $ocinstanceid,
                        $courseid,
                        $sourcecourseid,
                        $resultduplicate->duplicatedseries
                    );
                }
