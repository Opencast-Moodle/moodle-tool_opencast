...
if (!$transcriptionmanagementenabled) {
    redirect($redirecturl, get_string('transcriptionmanagementdisabled', 'tool_opencast'), null, notification::NOTIFY_ERROR);
}

if ($video->error || $video->video->processing_state != 'SUCCEEDED') {
    redirect($redirecturl, get_string('unabletodeletetranscription', 'tool_opencast'), null, notification::NOTIFY_ERROR);
}
