...
use tool_opencast\local\changeowner_form;
use core\output\notification;
...
if ($courseid == $SITE->id) {
    $redirecturl = new moodle_url('/admin/tool/opencast/overview.php', ['ocinstanceid' => $ocinstanceid]);
} else {
...
if (empty(get_config('tool_opencast', 'aclownerrole_' . $ocinstanceid))) {
    redirect($redirecturl, get_string('functionalitydisabled', 'tool_opencast'), null, notification::NOTIFY_ERROR);
}
...
if ($isseries) {
    ...
    if (!$series) {
        $str = get_string('series_does_not_exist_admin', 'tool_opencast', $identifier);
        redirect($redirecturl, $str, null, notification::NOTIFY_ERROR);
    }
    $title = $series->title;
    $acls = $series->acl;
    ...
    if ($video->error) {
        redirect($redirecturl, get_string('failedtogetvideo', 'tool_opencast'), null, notification::NOTIFY_ERROR);
    } else {
