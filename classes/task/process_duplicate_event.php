...
class process_duplicate_event extends adhoc_task {
    ...
    public function __construct() {
        $this->set_component('tool_opencast');
    }
    ...
    public function execute() {
        ...
        try {
            ...
            if (empty($duplicateworkflow)) {
                throw new moodle_exception('error_workflow_setup_missing', 'tool_opencast');
            }
            ...
            if (empty($data->seriesid)) { // Should not happen as seriesid is checked during restore.
                throw new moodle_exception('error_seriesid_taskdata_missing', 'tool_opencast', '', $a);
            }
            ...
            if (!$seriesid = $apibridge->get_stored_seriesid($course->id)) {
                throw new moodle_exception('error_seriesid_missing_course', 'tool_opencast', '', $a);
            }

            // ...and matches the id of task.
            if ($seriesid != $data->seriesid) {
                throw new moodle_exception('error_seriesid_not_matching', 'tool_opencast', '', $a);
            }

            // Check, whether series exists in opencast system.
            $series = $apibridge->get_default_course_series($course->id);
            if (!isset($series)) {
                throw new moodle_exception('error_seriesid_missing_opencast', 'tool_opencast', '', $a);
            }

            // Event checks.
            // Intentionally no check, whether event exists on opencast system as customer required (May be done by the opencast).
            if (empty($data->eventid)) {
                throw new moodle_exception('error_eventid_taskdata_missing', 'tool_opencast', '', $a);
            }

            // Workflow checks.
            if (!$apibridge->check_if_workflow_exists($duplicateworkflow)) {
                throw new moodle_exception('error_workflow_not_exists', 'tool_opencast', '', $a);
            }
            ...
            if (!$ocworkflowid) {
                throw new moodle_exception('error_workflow_not_started', 'tool_opencast', '', $a);
            }
            ...
            if ($data->schedulemodulecleanup == true && is_number($ocworkflowid) &&
                ...
                foreach ($data->episodemodules as $coursemoduleid => $oldepisodeid) {
                    ...
                    if (!$DB->record_exists('tool_opencast_ltiepisode_cu',
                        ...
                        $DB->insert_record('tool_opencast_ltiepisode_cu', $record);
                    }
                }
            }
